...
df = df[['DATE', 'Downtime (hrs)']]
df

#Now, we can convert the “Downtime (hrs)” data type to float
df = df.astype({"Downtime (hrs)": float})
...
data = pd.DataFrame(index=range(0,len(df)),columns=['DATE','Downtime (hrs)'])



for i in range(0,len(data)):
    data["DATE"][i]=df['DATE'][i]
    data["Downtime (hrs)"][i]=df["Downtime (hrs)"][i]
